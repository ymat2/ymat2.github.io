...
仮想環境を作って `pip` / `uv pip` / `uv add` で入れる。
...
```sh
pip install cython bottle brotli numpy scipy PyQt5
pip install ete3
ete3 upgrade-external-tools   # codeml, slrをここからインストールすることもできるらしい
```

### 遺伝研スパコン

2026/03/29 時点で `ete2` と `ete3` が利用可能:
