...
Windows では current user & current host の `.ps1` に PATH 設定が追記される。
...
`--app`
: デフォルトの挙動。アプリケーション用プロジェクトとして立ち上げる。

`--package`
: パッケージ用プロジェクトとして立ち上げる。
: `main.py` の代わりに `src/<project_name>/__init__.py`

`--lib`
: ライブラリ用プロジェクトとして立ち上げる。
: `--package` 同様の構造に、`__init__.py` + `py.typed`
