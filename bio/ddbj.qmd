...
- <https://sc.ddbj.nig.ac.jp/guides/using_general_analysis_division/ga_application/>

コマンドライン操作をともなうので、Windows では [PowerShell](/win/powershell.qmd) を使う。
[WSL](/win/wsl.qmd) や [Git for Windows](/win/0-dev-win.qmd) 同梱の Bash を使うのもオススメ。

事前に [SSH 公開鍵・秘密鍵](/cli/ssh.qmd)を作成しておく。
すでにある場合はそれを使ってもよい。

- [SSH公開鍵の作成方法](https://sc.ddbj.nig.ac.jp/application/ssh_keys/)
- [SSH公開鍵の設置方法](https://sc.ddbj.nig.ac.jp/application/ssh_copy_id/)


## ログイン

- <https://sc.ddbj.nig.ac.jp/guides/using_general_analysis_division/ga_login/>
- <https://sc.ddbj.nig.ac.jp/guides/using_general_analysis_division/ga_interactive_login/>

ゲートウェイノード &rarr; インタラクティブノードの順に `ssh` する。
ゲートウェイノードは `gw.ddbj.nig.ac.jp`, `gw2.ddbj.nig.ac.jp` の2つ、
インタラクティブノード `a001`, `a002`, `a003` の3つからいずれかを指定する。
...
インタラクティブノードではファイルの確認など軽い作業のみ行う。
大量のメモリ・CPU を使う作業をインタラクティブに行うには
[`srun`](https://slurm.schedmd.com/srun.html) コマンドを使って計算ノードを確保する:
...
基本的な流れは Grid Engine と同じで、ジョブスクリプトを書いて `sbatch example.sh` するだけ:
...
Grid Engine でいう `#$ -cwd` (カレントディレクトリの設定) と `#$ -V` (環境変数の引継ぎ) は、Slurm ではデフォルトらしい。

インタープリタの指定オプションはない
([?](https://cvw.cac.cornell.edu/slurm/basics/execution-sbatch))
ので、shebang で指定する必要がある:
`#!/bin/bash`, `#!/bin/csh`, `#!/usr/bin/python`
...
同時に投入されるタスク数の上限は `%N` で制御する (Grid Engine でいう `-tc N`)。
例えば1から1000までの並列ジョブで、同時に走るジョブ数の上限を10にするには:
`#SBATCH -a 1-1000%10`
...
Slurm には Grid Engine でいうパラレルジョブに直接該当する機能はない。
