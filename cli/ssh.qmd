...
後述の SCP や Rsync を用いたファイル送受信時も記述を省略できて便利:
...
```sh
scp path/to/file_to_send <user>@<hostname>:path/to/destination
```
...
送信元と送信先の差分を検出してファイルの同期を行う。
SSH でリモートにファイル送信・同期を行うほか、ローカルどうしのバックアップにも使える。

```sh
rsync -auvz path/to/file_to_send <user>@<hostname>:path/to/destination
```
...
`-l`, `--links`
: symlinks を symlinks のままにする。

`-p`, `--perms` / `-t`, `--times` / `-g`, `--group` / `-o`, `--owner`
: それぞれファイルの permissions、更新タイムスタンプ、グループ、オーナーの情報を保持

`-D` = `--devices --specials`
: デバイスファイル、特殊ファイルを保持

`-u`, `--update`
:	送信先側で更新されているファイルをスキップする。
