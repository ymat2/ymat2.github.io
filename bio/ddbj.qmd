...
コマンドライン操作をともなうので、Windows では
[WSL](/win/wsl.qmd) や [Git for Windows](/win/0-dev-win.qmd)
を入れておくのがオススメ。

事前に [SSH 公開鍵・秘密鍵](/cli/ssh.qmd)を作成しておく。
すでにある場合はそれを使ってもよい。
[SSH公開鍵の作成方法](https://sc.ddbj.nig.ac.jp/application/ssh_keys/)や
[SSH公開鍵の設置方法](https://sc.ddbj.nig.ac.jp/application/ssh_copy_id/)を参照。
...
ゲートウェイノード &rarr; インタラクティブノードの順に `ssh` ログインする。
